...
namespace Repositorio
{
    public class RepositorioFuncionarios
    {
        ...
        {
            return this.Funcionarios.Where(func => func.Cargo.Equals(cargo)).ToList();
        }

        public IList<Funcionario> OrdenadosPorCargo()
        {
            var listaOrdernada = this.Funcionarios.OrderBy(func => func.Cargo.Titulo).ThenBy(func => func.Nome).ToList();
            return listaOrdernada;
        }

        public IList<Funcionario> BuscarPorNome(string nome)
        {
            return this.Funcionarios.Where(func => func.Nome.Contains(nome)).ToList();
        }        
        ...
        {
            DateTime idadeAprox = DateTime.Now.AddYears(-idade);
            return this.Funcionarios.Where(func => (idadeAprox.AddYears(-5) <= func.DataNascimento && idadeAprox.AddYears(5) >= func.DataNascimento)).ToList();
        }        

        public double SalarioMedio(TurnoTrabalho? turno = null)
        {
            return Funcionarios.Where(func => (turno == null || func.TurnoTrabalho.Equals(turno))).Select(func => func.Cargo.Salario).Average();
        }

        public IList<Funcionario> AniversariantesDoMes()
        {
            return this.Funcionarios.Where(func => func.DataNascimento.Month == DateTime.Now.Month).ToList();
        }

        public IList<dynamic> BuscaRapida()
        {
            return this.Funcionarios.Select(f => new
            {
